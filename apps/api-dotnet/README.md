...
## DynamoDB-backed households

Households are stored in the `HOUSEHOLDS_TABLE_NAME` DynamoDB table. Cognito JWT validation
is enabled for `/v1` routes; the legacy `X-User-Id` development bypass is intentionally not
supported.
