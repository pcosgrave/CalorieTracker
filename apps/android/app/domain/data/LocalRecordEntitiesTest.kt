...
class LocalRecordEntitiesTest {
    ...
    fun test_FoodRecordEntity_creation() {
        ...
        assertEquals("2024-01-01T12:00:00Z", record.updatedAt)
        assertNull(record.deletedAt)
        assertEquals("device-1", record.originDeviceId)
    ...
    fun test_BarcodeAliasEntity_creation() {
        ...
        assertEquals("food-1", entity.productId)
        assertEquals(1, entity.version)
        assertEquals("device-1", entity.originDeviceId)
    ...
    fun test_DiaryRecordEntity_creation() {
        ...
        assertEquals("2024-01-01T12:00:00Z", entity.loggedAt)
        assertEquals(1, entity.version)
        assertEquals("pending_push", entity.syncStatus)
    }
    ...
    fun test_SyncOutboxEntity_creation() {
        ...
        assertEquals("food-1", outbox.recordId)
        assertEquals("UPSERT", outbox.operation)
        assertEquals("device-1", outbox.deviceId)
        assertNull(outbox.payloadJson)
        assertNull(outbox.baseVersion)
    }
    ...
    fun test_WeightRecordEntity_creation() {
        ...
        assertEquals("2024-01-01T12:00:00Z", record.loggedOn)
    ...
    @Test
    fun test_WeightRecordEntity_defaults() {
        val record = WeightRecordEntity(
            recordId = "weight-default",
            ownerUserId = "user-default",
            loggedOn = "2024-01-01T00:00:00Z",
            weightKg = 68.5,
            updatedAt = "2024-01-01T00:00:00Z"
        )

        assertEquals(68.5, record.weightKg)
    }

    @Test
    fun test_SyncMetadata_status_values() {
        // Check sync status string values
        assertTrue("pending_push" == SyncStatus.PendingPush.name.toLowerCase())
        assertTrue("synced" == SyncStatus.Synced.name.toLowerCase())
        assertTrue("sync_error" == SyncStatus.SyncError.name.toLowerCase())
        assertTrue("local_only" == SyncStatus.LocalOnly.name.toLowerCase())
    }
    ...
    @Test
    fun test_SyncOutboxEntity_different_operations() {
        val operations = listOf(
            SyncOutboxEntity(
                changeId = "change-upsert",
                entityType = "FOOD_PRODUCT",
                recordId = "food-1",
                operation = "UPSERT",
                changedAt = "2024-01-01T12:00:00Z",
            ...
            SyncOutboxEntity(
                ...
                recordId = "entry-1",
                operation = "DELETE",
                changedAt = "2024-01-01T12:01:00Z",
                deviceId = "device-1"
            )
        )

        assertEquals("UPSERT", operations[0].operation)
        assertEquals("DELETE", operations[1].operation)
    }
