...
private val DraftsSaver = listSaver<List<PhotoFoodDraft>, String>(
...
fun PhotoFoodScreen(
    ...
    var drafts by rememberSaveable(stateSaver = DraftsSaver) { mutableStateOf(initialEntries.map {
    ...
    var mealName by rememberSaveable { mutableStateOf((initialEntries.firstOrNull()?.meal ?: inferMealForTime(LocalTime.now())).name) }
    var busy by remember { mutableStateOf(false) }
    var saving by remember { mutableStateOf(false) }
    var analysisJob by remember { mutableStateOf<Job?>(null) }
    var status by remember { mutableStateOf("") }
    var error by remember { mutableStateOf<String?>(null) }
    val analyzer = remember { LocalPhotoFoodAnalyzer() }
    ...
    fun analyze(bitmap: Bitmap) {
        ...
        analysisJob = scope.launch(start = CoroutineStart.UNDISPATCHED) {
            try {
                drafts = withTimeout(300_000) { analyzer.analyze(bitmap, foods) { status = it } }
    ...
    if (creating != null) {
        ...
        key(creating.id) {
            ...
            AddIngredientScreen(
                barcodeLookupResult = lookup, isLookingUpBarcode = lookingUp,
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(14.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (drafts.isNotEmpty()) {
            ...
            if (!busy) {
                ...
                MealPicker(Meal.valueOf(mealName), { mealName = it.name })
                DatePillsRow(LocalDate.parse(selectedDate), LocalDate.now(), { if (!it.isAfter(LocalDate.now())) selectedDate = it.toString() })
            }
            ...
            }) { Text("Save list to ${Meal.valueOf(mealName).label}") }
        }
    }
}
...
private fun PhotoFoodEditor(
    ...
    val portionCalories = grams?.let { amount ->
        if (matched != null) convertAmount(amount, draft.amountUnit, matched.servingUnit)?.let {
    ...
    Card(
        ...
        Column(Modifier.background(colors.surfaceVariant.copy(alpha = 0.3f)).padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (matched == null) {
                ...
                val calories = amount?.let { convertAmount(it, draft.amountUnit, matched.servingUnit) }?.let { it / matched.servingQuantity * matched.nutrients.calories }
